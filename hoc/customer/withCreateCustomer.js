...
import * as gqlQuery from 'shared/graphql/typedefs/customer.gql';
...
export default (WrappedComponent: any) => {
  let withCreateCustomer = (props: any) => {
    ...
    const createCustomer = async ({ companyId, name, govNumber, ...rest }) => {
      ...
      let parsedRes = utils.handleMutationResponse(res, 'createCustomer');
      if (parsedRes.success) {
        if (rest) {
          ...
          let parsedTrader = utils.handleMutationResponse(
            traderRes,
            'updateTrader'
          );
